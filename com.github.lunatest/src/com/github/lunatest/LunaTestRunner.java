...
/**
 * Runner used to pass LunaTest tests using JUnit 4. This class must be used with the RunWith annotation and the test class must have a method
 * <code>public static void suite(LunaTestRunner runner)</code> where <code>runner</code> is the runner instance which will run the tests. For
 * example:
 * 
 * <pre>
 ...
 * <li>There is still some outputs on console</li>
 * </ul>
 * 
 * @author jdesgats
 */
...
public class LunaTestRunner extends Runner {
	...
	/**
	 * Runs the Lua tests: builds test description and put results on events queue. This must be ran before by constructor because there is no
	 * "description" phase on LunaTest so the description is known only at run time.
	 */
	...
	/**
	 * Utility function to make given file name a loadable module (by require). Can be used to make script under test available to test script itself
	 * without hacking paths.
	 * 
	 * @param filename
	 *            File name to load.
	 * @param modname
	 *            Module name (the argument for require)
	 */
	 ...
	 * 
	 * @param suiteFile
	 *            File which contains tests source
	 * @param suiteName
	 *            Test suite name
	 */
